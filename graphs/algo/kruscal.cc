...
EdgeList* Kruscal::getSpaingTree(EdgeList* graph) {
    ...
    int edges_count = graph->getEdgesCount();

    std::vector<std::pair<int, std::pair<int, int>>> copied_graph;
    copied_graph.reserve(edges_count);
    std::for_each(list->begin(), list->end(), [&copied_graph](auto& edge) {
        copied_graph.push_back({edge.second, {edge.first.first, edge.first.second}});
    });
    ...
    return new EdgeList(std::move(result), false, true, vertices_count);
}
