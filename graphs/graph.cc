...
int Graph::checkEuler(bool& circleExist) {
  ...
  int vertice;
  if (adj_list) {
    vertice = euler_circle_.checkEuler(*adj_list, circleExist);
    adj_list->clearGraph();
    delete adj_list;
  } else {
    vertice = euler_circle_.checkEuler(
        *static_cast<AdjacencyList*>(graph_repr_), circleExist);
  }
  return vertice;
}
...
std::vector<int> Graph::getEuleranTourFleri() {
  ...
  std::vector<int> path;
  if (adj_list) {
    path = fleri_.getEuleranTour(*adj_list);
    adj_list->clearGraph();
    delete adj_list;
  } else {
    path = fleri_.getEuleranTour(*static_cast<AdjacencyList*>(graph_repr_));
  }
  return path;
}
...
EdgeList* Graph::convertToNewEdgeList() {
  switch (graph_type_) {
    case GraphType::AdjacencyList:
      return static_cast<AdjacencyList*>(graph_repr_)->getNewListOfEdges();

    case GraphType::AdjacencyMatrix:
      return static_cast<AdjacencyMatrix*>(graph_repr_)->getNewListOfEdges();
